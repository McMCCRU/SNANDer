...
static_assert((__EMSCRIPTEN_major__ * 100 * 100 + __EMSCRIPTEN_minor__ * 100 + __EMSCRIPTEN_tiny__) >= 30148,
...
val makeControlTransferPromise(const val& dev, libusb_control_setup* setup)
{
    ...
    {
        return dev.call<val>("controlTransferOut", params, getUnsharedMemoryView(setup + 1, setup->wLength));
    }
}
...
struct CachedDevice
{
    ...
    static val initFromDevice(val&& web_usb_dev, libusb_device* libusb_dev)
    {
        ...
        val result = co_await cachedDevicePtr->initFromDeviceWithoutClosing(libusb_dev, must_close);
        if (must_close)
        {
            co_await_try(cachedDevicePtr->safeOpenCloseAssumingMainThread(OpenClose::Close));
        }
    ...
    uint8_t getActiveConfigValue() const
    {
        return runOnMain(
            [&]
            {
                ...
                return web_usb_config.isNull() ? 0 : web_usb_config["configurationValue"].as<uint8_t>();
            });
    }

    usbi_configuration_descriptor* getConfigDescriptor(uint8_t config_id)
    {
        return config_id < configurations.size() ? configurations[config_id].get() : nullptr;
    }

    usbi_configuration_descriptor* findConfigDescriptorByValue(uint8_t config_id) const
    {
    ...
    int copyConfigDescriptor(const usbi_configuration_descriptor* config, void* buf, size_t buf_len)
    {
    ...
    {
        return ::awaitOnMain([&] { return CaughtPromise(device.call<val>(methodName, std::forward<Args>(args)...)); })
            .error;
    }
    ...
    {
        return val::take_ownership(usbi_em_device_safe_open_close(device.as_handle(), static_cast<bool>(open)));
    }

    int safeOpenCloseOnMain(OpenClose open)
    {
        return ::awaitOnMain([this, open] { return safeOpenCloseAssumingMainThread(open); }).error;
    }
...
private:
    ...
    CaughtPromise requestDescriptor(libusb_descriptor_type desc_type, uint8_t desc_index, uint16_t max_length) const
    {
    ...
    val initFromDeviceWithoutClosing(libusb_device* dev, bool& must_close)
    {
        ...
        {
            auto result = co_await_try(requestDescriptor(LIBUSB_DT_DEVICE, 0, LIBUSB_DT_DEVICE_SIZE));
        ...
        for (uint8_t j = 0; j < configurations_len; j++)
        {
            ...
            auto result = co_await_try(requestDescriptor(LIBUSB_DT_CONFIG, j, MAX_CTRL_BUFFER_LENGTH));
            if (auto error = getTransferStatus(result))
            {
                co_return error;
            }
            ...
            auto& config = configurations.emplace_back((usbi_configuration_descriptor*) ::operator new(configLen));
...
{
    thread_local const val SessionIdSymbol = val::global("Symbol")(val("libusb.session_id"));
...
val getDeviceList(libusb_context* ctx, discovered_devs** devs)
{
    ...
    auto web_usb_devices = co_await_try(val::global("navigator")["usb"].call<val>("getDevices"));
    for (auto&& web_usb_device : web_usb_devices)
    {
        ...
        if (dev == NULL)
        {
            ...
            auto statusVal = co_await CachedDevice::initFromDevice(std::move(web_usb_device), dev);
            if (auto error = statusVal.as<int>())
            {
                usbi_err(ctx, "failed to read device information: %s", libusb_error_name(error));
...
int em_get_device_list(libusb_context* ctx, discovered_devs** devs)
{
    ...
    return awaitOnMain([ctx, devs] { return IntPromise(getDeviceList(ctx, devs)); }).error;
}
...
void em_close(libusb_device_handle* handle)
{
    ...
    {
        usbi_err(handle->dev->ctx, "failed to close device: %s", libusb_error_name(error));
    }
}
...
int em_get_config_descriptor(libusb_device* dev, uint8_t config_id, void* buf, size_t len)
{
    auto& cached_device = *WebUsbDevicePtr(dev);
...
int em_get_configuration(libusb_device_handle* dev_handle, uint8_t* config_value)
{
    *config_value = WebUsbDevicePtr(dev_handle)->getActiveConfigValue();
    return LIBUSB_SUCCESS;
}

int em_get_config_descriptor_by_value(libusb_device* dev, uint8_t config_value, void** buf)
{
    auto& cached_device = *WebUsbDevicePtr(dev);
    if (auto config = cached_device.findConfigDescriptorByValue(config_value))
    {
...
{
    return WebUsbDevicePtr(handle)->awaitOnMain("selectAlternateInterface", iface, altsetting);
}
...
int em_submit_transfer(usbi_transfer* itransfer)
{
    return runOnMain(
        [itransfer]
        {
            ...
            auto& web_usb_device = WebUsbDevicePtr(transfer->dev_handle)->getDeviceAssumingMainThread();
            val transfer_promise;
            switch (transfer->type)
            {
            case LIBUSB_TRANSFER_TYPE_CONTROL:
            {
                transfer_promise = makeControlTransferPromise(web_usb_device,
            ...
            case LIBUSB_TRANSFER_TYPE_INTERRUPT:
            {
                ...
                {
                    transfer_promise = web_usb_device.call<val>("transferIn", endpoint, transfer->length);
                }
                else
                {
                    auto data = getUnsharedMemoryView(transfer->buffer, transfer->length);
                    transfer_promise = web_usb_device.call<val>("transferOut", endpoint, data);
                }
...
{
    libusb_transfer_status status = runOnMain(
        [itransfer]
        {
            auto transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
    ...
    return status == LIBUSB_TRANSFER_CANCELLED ? usbi_handle_transfer_cancellation(itransfer)
